...
err_code_t flash_erase_fw_block(uint32_t addr, uint32_t length )
{
  int i = 0;
  err_code_t err_code = OK;
  HAL_StatusTypeDef HAL_FLASHStatus = HAL_OK;
...
for (i = 0; i < sector_cnt; i++)
{
  ...
  pEraseInit.PageAddress = (128+i)*2*1024 + FLASH_BASE + addr;
...
err_code_t flash_erase_sector(uint32_t sector)
{
  ...
  pEraseInit.TypeErase = FLASH_TYPEERASE_PAGES;
  pEraseInit.PageAddress = sector*2*1024 + FLASH_BASE;
  pEraseInit.NbPages = 1;
